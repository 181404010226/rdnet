...
class CustomCIFAR10(torchvision.datasets.CIFAR10):
    def __init__(self, *args, **kwargs):
        ...
        self.cat_dog = [3, 5]  # cat, dog
        self.deer_horse = [4, 7]  # deer, horse
        ...
        for img, target in zip(self.data, self.targets):
            if target in self.cat_dog + self.deer_horse:
                # 添加两份，以平衡数据集
                self.filtered_data.append(img)
                self.filtered_targets.append(0 if target in self.cat_dog else 1)
                self.filtered_data.append(img)
                self.filtered_targets.append(0 if target in self.cat_dog else 1)
