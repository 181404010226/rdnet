...
def main():
    ...
    class CustomCIFAR10(torchvision.datasets.CIFAR10):
        def __init__(self, *args, **kwargs):
            ...
            self.airplane_ship = [0, 8]  # airplane, ship
            self.automobile_truck = [1, 9]  # automobile, truck
            ...
            for img, target in zip(self.data, self.targets):
                if target in self.airplane_ship + self.automobile_truck:
                    # 添加两份，以平衡数据集
                    self.filtered_data.append(img)
                    self.filtered_targets.append(0 if target in self.airplane_ship else 1)
                    self.filtered_data.append(img)
                    self.filtered_targets.append(0 if target in self.airplane_ship else 1)
